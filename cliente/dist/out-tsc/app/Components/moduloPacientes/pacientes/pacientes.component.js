...
var PacientesComponent = /** @class */ (function () {
    function PacientesComponent(servicioPersona, servicioTS, servicioPaciente, dialog, servicioEventos, router, servicioGrupoEtnico, servicioOcupacion) {
        ...
        this.displayedColumns = ['Acciones', 'Rut', 'Nombre', 'Tipo Sangre', 'Grupo Etnico', 'Ocupacion'];
    ...
    */
    PacientesComponent.prototype.actualizarPersonas = function () {
        ...
        this.servicioPersona.getPersonas().subscribe(function (data) {
            ...
            console.log(_this.totalPersonas);
            _this.totalTS = [];
            _this.servicioTS.getTipoSangres().subscribe(function (data) {
                var todo = data;
                todo = todo.data;
                _this.totalTS = todo;
                _this.totalGruposEtnicos = [];
                _this.servicioGrupoEtnico.getGrupoEtnicos().subscribe(function (data) {
                    var todo = data;
                    todo = todo.data;
                    _this.totalGruposEtnicos = todo;
                    _this.totalOcupaciones = [];
    ...
    PacientesComponent.prototype.desplegarFichaPaciente = function (paciente) {
        var a = JSON.parse(JSON.stringify(paciente));
        this.pasarStringId(a);
        var dialogRef = this.dialog.open(verfichamedica_component_1.VerFichaMedicaComponent, {
            width: '1000px',
            height: '700px',
            data: {
                paciente: a
    ...
    PacientesComponent.prototype.desplegarFichaAtencionPaciente = function (paciente) {
        var a = JSON.parse(JSON.stringify(paciente));
        this.pasarStringId(a);
    ...
    PacientesComponent.prototype.filtrarPacientesRegistrados = function () {
        for (var i = 0; i < this.totalPacientes.length; i++) {
